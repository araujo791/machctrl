...
const TASKS: Omit<CleanTask, 'status'>[] = [
  ...
  { id: 'thumb-cache',    label: 'Cache de Miniaturas',  description: 'Limpa thumbnails (~/.cache/thumbnails)',         needsRoot: false },
  { id: 'coredumps',      label: 'Core Dumps',           description: 'Remove arquivos de crash do sistema',           needsRoot: true  },
  ...
  { id: 'trash',          label: 'Lixeira',              description: 'Esvazia a lixeira (~/.local/share/Trash)',      needsRoot: false },
]
...
export function CleanerPanel({ sendCommand, addMessageListener }: Props) {
  ...
  // Registra listener para respostas do backend
  useEffect(() => {
  ...
  const execTask = useCallback((taskId: string): Promise<{ bytes: number }> => {
    return new Promise((resolve) => {
      ...
      // Registra resolver para quando a resposta chegar
